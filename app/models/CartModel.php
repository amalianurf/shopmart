...
class CartModel {
    ...
    public function getAll() {
        $query = "SELECT transaksi.*, produk.*, users.*, transaksi.*, COUNT(transaksi.id_produk) AS total_produk, SUM(produk.harga) AS total_harga FROM transaksi LEFT JOIN users ON users.id = transaksi.id_user LEFT JOIN produk ON produk.id = transaksi.id_produk WHERE transaksi.status='proses' GROUP BY transaksi.id_user, transaksi.id_produk";
        $this->db->query($query);
        $row = $this->db->result();
        if (!empty($row)) {
            return $row;
    ...
    public function add($data) {
        try {
            $this->db->query("INSERT INTO transaksi (id_user, id_produk) VALUES (?, ?)");
